...
function buildExtractionPrompt(resume: string): string {
  return [
    ...
    "- name: the candidate's own name, the line the resume opens with. Never",
    "  a project, employer, school or section heading.",
    ...
    "  description is the rest of the project's own title line, which is",
    '  usually its stack ("Python, SQLite, OAuth") — copy it without the',
    '  dash or pipe that separates it from the name, or "" if the line is',
    "  only a name. url only if the resume prints a link for it.",
